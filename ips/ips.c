...
{
    png_uint_32 x, y;
    png_bytep destination_pixel;
    int channel, Gx, Gy; 
    ips_raw_image_t *input_image = task->input_image;
    ips_raw_image_t *output_image = task->output_image;
    unsigned int channels = output_image->channels;
	int matrixxGradient[3][3];
    for (y = task->row_index_to_process; y < task->last_row_index_to_process; y++) {
        for (x = 0; x < output_image->width; x++) {
			destination_pixel = &(output_image->rows[y][x * channels]);
			if(y != 0 && y != output_image->height - 1 && x != 0 && x != output_image->width - 1) {
				...
				for (channel = 0; channel < 3; ++channel) {
					png_bytep p1 = &(input_image->rows[y - 1][(x - 1) * channels]);	a1 += p1[channel]; 
					png_bytep p2 = &(input_image->rows[y - 1][(x) * channels]);		a2 += p2[channel]; 
					png_bytep p3 = &(input_image->rows[y - 1][(x + 1) * channels]);	a3 += p3[channel]; 
					png_bytep p4 = &(input_image->rows[y][(x - 1) * channels]);		a4 += p4[channel]; 
					png_bytep p5 = &(input_image->rows[y][(x) * channels]);			a5 += p5[channel]; 
					png_bytep p6 = &(input_image->rows[y][(x + 1) * channels]);		a6 += p6[channel]; 
					png_bytep p7 = &(input_image->rows[y + 1][(x - 1) * channels]);	a7 += p7[channel]; 
					png_bytep p8 = &(input_image->rows[y + 1][(x) * channels]);		a8 += p8[channel]; 
					png_bytep p9 = &(input_image->rows[y + 1][(x + 1) * channels]);	a9 += p9[channel]; 
				}
				a1 = a1 / 3.0; a2 = a2 / 3.0; a3 = a3 / 3.0; a4 = a4 / 3.0;
				a5 = a5 / 3.0; a6 = a6 / 3.0; a7 = a7 / 3.0; a8 = a8 / 3.0; a9 = a9 / 3.0;
				Gx = (a7 + a8 * 2 + a9) - (a1 + a2 * 2 + a3);
...
GLuint ips_create_shader_program(char *vertex_shader_source, char *fragment_shader_source)
{
    ...
    if (!fragment_shader_source) {
        fprintf(stderr, "Failed to read a fragment shader file \"%s\"\n", Fragment_Shader_Path);
        return shader_program;
    }

    vertex_shader_object = ips_compile_shader(vertex_shader_source, GL_VERTEX_SHADER);

    if (vertex_shader_object) {
        fragment_shader_object = ips_compile_shader(fragment_shader_source, GL_FRAGMENT_SHADER);
...
void ips_update_matrices(int window_width, int window_height)
{
    ...
    float aspect_ration = fabsf((float) window_width /(float) window_height);

    projection_matrix = glm::ortho<float>(-(camera_zoom * aspect_ration), camera_zoom * aspect_ration,
            -camera_zoom, camera_zoom, 0.1f, 100);
...
void ips_start(char *dropped_file_path)
{
    ...
    shader_program = ips_create_shader_program(ips_utils_read_text_file(Vertex_Shader_Path), ips_utils_read_text_file(Fragment_Shader_Path));
    ...
    for (;;) {
        ...
        ips_render_quad(shader_program, vertex_array_object, texture);
